...
/**
 * Identity provider mapper that syncs the user's groups received from an
 * external IdP into Keycloak groups.
 *
...
public class ClaimToGroupMapper extends AbstractClaimMapper {
    ...
    static {
        ...
        property.setHelpText("Only sync groups that contain this text in their name. If empty, sync all groups.");
        property.setType(ProviderConfigProperty.STRING_TYPE);
        CONFIG_PROPERTIES.add(property);
        ...
        property.setHelpText("Indicates if missing groups must be created in the realm. Otherwise, they will be ignored.");
    ...
    public static List<String> getClaimValue(BrokeredIdentityContext context, String claim) {
        ...
        Object roles = AbstractJsonUserAttributeMapper.getJsonValue(profileJsonNode, claim);
        if (roles == null) {
    ...
    private void syncGroups(RealmModel realm, UserModel user, IdentityProviderMapperModel mapperModel,
        ...
        if (newGroupsList.isEmpty() && !clearRolesIfNone) {
            logger.debugf("Realm [%s], IdP [%s]: no group claim (claim name: [%s]) for user [%s], ignoring...",
                    realm.getName(), mapperModel.getIdentityProviderAlias(), groupClaimName, user.getUsername());
            return;
        }
        ...
        Set<String> newGroupEntries = newGroupsList.stream()
                .filter(entry -> {
                    if (isEmpty(containsText)) return true;
                    String groupName = entry.contains(":") ? entry.split(":", 2)[0] : entry;
                    return groupName.contains(containsText);
                })
                .collect(Collectors.toSet());
        ...
        for (GroupModel group : getGroupsToBeRemoved(currentGroups, newGroups))
            user.leaveGroup(group);
        for (GroupModel group : getGroupsToBeAdded(currentGroups, newGroups))
    ...
    private static Set<GroupModel> getGroupsToBeRemoved(Set<GroupModel> currentGroups, Set<GroupModel> newGroups) {
        Set<GroupModel> result = new HashSet<>(currentGroups);
        result.removeAll(newGroups);
        return result;
    }

    private static Set<GroupModel> getGroupsToBeAdded(Set<GroupModel> currentGroups, Set<GroupModel> newGroups) {
        Set<GroupModel> result = new HashSet<>(newGroups);
        result.removeAll(currentGroups);
        return result;
    }
