...
public class ClaimToGroupMapper extends AbstractClaimMapper {
    ...
    private void syncGroups(KeycloakSession session, RealmModel realm, UserModel user, IdentityProviderMapperModel mapperModel, BrokeredIdentityContext context) {
        ...
        Set<GroupModel> current = user.getGroupsStream()
                ...
                .collect(Collectors.toSet());
        Set<GroupModel> target = getOrCreateGroups(session, realm, desiredNames, createGroups);
    ...
    private Set<GroupModel> getOrCreateGroups(KeycloakSession session, RealmModel realm, Set<String> names, boolean create) {
